<?php
class Db_PDOPROXY extends DbObject
{
    ...
    public $pool_server_connected;


    /**
     * Db_PDOPROXY constructor.
     * @param string $prefix
     */
    public function __construct($prefix = "mysql")
    {
        $this->prefix = $prefix;
        if (hlw_lib_Constant::RUNTIME_ENVIRONMENT == 'product') {
            $this->isShowDbError = false;
        }
    }
    ...
    public function init($params = array(), $table = '')
    {
        ...
        if (!empty($dbConfig)) {
            ...
            if (!isset(Db_PDOPROXY::$globals[$this->key])) {
                Db_PDOPROXY::$globals[$this->key] = "";
            }
        } else {
            $log_data = array(
                'table' => $table,
    ...
    function query($table, $sql, $bind1 = array(), $bind2 = array(), $params = array())
    {
        ...
        //2010.12.28 增加查询时加锁模式
        $lock = isset($params['lock']) ? ' ' . $params['lock'] : '';
        $stmt = $this->getPodObject($table, $params)->prepare($sql . $lock);


        if (!$stmt) {
            $this->error['code'] = Db_PDOPROXY::$globals[$this->key]->errorCode();
            if ($this->isShowDbError) {
                $this->error['msg'] = Db_PDOPROXY::$globals[$this->key]->errorInfo();
            } else {
                $this->error['msg'] = "mysql error";
                $this->addLog('Url:' . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'] . ' mysqlerror:' . json_encode(Db_PDOPROXY::$globals[$this->key]->errorInfo()));
            }
        ...
        try {
            ...
            if ($this->isShowDbError) {
                ...
                $this->error['msg'] = "mysql error";
                $this->addLog('Url:' . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'] . ' mysqlerror:' . json_encode($e));
            }
            $this->debugMessage('<span style="color:red;">', $this->error, '</span>');
        ...
        if ($exec_result) {
            ...
            //如果是写入语句,execute后不应该再fetchALL
            //原生PDO,$db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);后也会抛异常
            //连接池在这里抛异常的话,会中断事务
            $is_not_write = self::isNotWrite($sql);
            $rtn = $is_not_write ? $stmt->fetchAll(PDO::FETCH_ASSOC) : array();

            //释放连接回到池子里,强制主库、事务中的情况暂时不处理,rshutdown/mshutdown阶段会自动释放
            if (null === self::$dbentry) {
                $stmt->release();
            }
    ...
    {
        return Db_PDOPROXY::$globals[$this->key]->lastInsertId();
    }
    ...
    {
        if (empty(Db_PDOPROXY::$globals[$this->key]) || $forceReconnect) {
            if (!empty(Db_PDOPROXY::$globals[$this->key])) {
                unset(Db_PDOPROXY::$globals[$this->key]);
            }
            try {

                Db_PDOPROXY::$globals[$this->key] = new PDOProxy($this->prefix . ":dbname=" . $this->database . ";host=" . $this->host . ";port=" . $this->port,
        ...
        if (!empty($this->charset)) {
            try {
                Db_PDOPROXY::$globals[$this->key]->exec("SET NAMES " . $this->charset);
            } catch (Exception $e) {

                if (extension_loaded('connect_pool')) {

                    //pool服务正常,但无法连接DB,写入日志
                    $logStr = 'Host:' . $_SERVER['SERVER_NAME'] . ' Url:' . $_SERVER['REQUEST_URI']
                        . 'db error :' . $this->database . ',' . $this->dtype . ',' . $e->getMessage();
    ...
    public function __quote($condition, $split = "AND", &$bind)
    {
        ...
        $this->getFilter($condiStr);//by 哪吒 2016-03-23
        return $condiStr;
    }
    ...
    public function getPodObject($table, $params = array('type' => 'query'))
    {
        ...
        if (empty($params['dbinfo'])) {
            ...
            if (empty(Db_PDOPROXY::$globals[$this->key])) {
                ...
                $this->debugMessage("connect state : " . PHP_EOL,
                    Db_PDOPROXY::$globals[$this->key]->getAttribute(PDO::ATTR_CONNECTION_STATUS));

                //如果已经有连接，检查连接状态
                /*
                if(!Db_PDOPROXY::$globals[$this->key]->getAttribute(PDO::ATTR_CONNECTION_STATUS)) {
        ...
        //var_dump(Db_PDOPROXY::$globals[$this->key]->getAttribute(PDO::ATTR_CONNECTION_STATUS));

        //echo $this->key . " \n<br>";

        return Db_PDOPROXY::$globals[$this->key];
    ...
    public function beginTransaction($table, $params = array('type' => 'main'))
    {
        $this->setDbEntry();
        try {
            $rtn = $this->getPodObject($table, $params)->beginTransaction();
        } catch (Exception $e) {
            return false;
        }
    ...
    {
        try {
            $rtn = $this->getPodObject($table, $params)->commit();
        } catch (Exception $e) {
            return false;
        }
    ...
    {
        try {
            $rtn = $this->getPodObject($table, $params)->rollBack();
        } catch (Exception $e) {
            return false;
        }
    ...
    {
        //若为连接池方式调用,主动将持有的连接放回池子里
        if (extension_loaded('connect_pool') && is_array(Db_PDOPROXY::$globals) && !empty(Db_PDOPROXY::$globals)) {
            foreach (Db_PDOPROXY::$globals as $v) {
                if (is_object($v)) {
                    $v->release();
                }
            }
        }
        Db_PDOPROXY::$globals = null;
    }
    ...
    /*
     * @param string 字符串
     * 记录危险sql 进行安全分析 by 哪吒 2016-03-23
     */
    ...
    private static function isNotWrite($sql) {
